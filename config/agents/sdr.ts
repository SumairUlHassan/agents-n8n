...
export const sdrAgentConfig: AgentConfig = {
  ...
  workflowNodes: [
    ...
    { id: "n4", label: "Company Data Enrichment", type: "action", description: "Enriches leads with funding, tech stack, and headcount", modelOrTool: "Clearbit API", position: { x: 560, y: 150 }, targetIds: ["n5"] },
    { id: "n5", label: "Website Research", type: "action", description: "Scrapes prospect website for recent news & tech initiatives", modelOrTool: "Firecrawl Scraper", position: { x: 730, y: 150 }, targetIds: ["n6"] },
    { id: "n6", label: "ICP Qualification", type: "agent", description: "Evaluates fit against Ideal Customer Profile (0-100 score)", modelOrTool: "Gemini 1.5 Pro", position: { x: 900, y: 150 }, targetIds: ["n7"] },
    { id: "n7", label: "Qualified Lead Branch", type: "decision", description: "Routes lead based on ICP score (>= 75 Qualified)", modelOrTool: "Condition Router", position: { x: 1070, y: 150 }, targetIds: ["n8"] },
    { id: "n8", label: "Personalization Agent", type: "agent", description: "Generates custom hook based on recent news & role pain points", modelOrTool: "Gemini Flash AI", position: { x: 1240, y: 150 }, targetIds: ["n9"] },
    { id: "n9", label: "Email Generator", type: "action", description: "Drafts 3-step cold email sequence tailored to prospect", modelOrTool: "Copy Engine", position: { x: 1410, y: 150 }, targetIds: ["n10"] },
    { id: "n10", label: "Compliance Check", type: "action", description: "Checks CAN-SPAM, unsubscribe link & tone guidelines", modelOrTool: "Guardrails API", position: { x: 1580, y: 150 }, targetIds: ["n11"] },
    { id: "n11", label: "Send Email", type: "action", description: "Dispatches email via Smartlead/Instantly platform", modelOrTool: "SendGrid / SMTP", position: { x: 1750, y: 150 }, targetIds: ["n12"] },
    { id: "n12", label: "Wait for Reply", type: "action", description: "Monitors inbox for prospect response", modelOrTool: "Email Poller", position: { x: 1920, y: 150 }, targetIds: ["n13"] },
    { id: "n13", label: "Reply Classification", type: "agent", description: "Classifies incoming reply (Interested, Objection, OOO, Unsubscribe)", modelOrTool: "NLP Classifier", position: { x: 2090, y: 150 }, targetIds: ["n14"] },
    { id: "n14", label: "Interested Branch", type: "decision", description: "Branches based on classification", modelOrTool: "Intent Router", position: { x: 2260, y: 150 }, targetIds: ["n15", "n16"] },
    { id: "n15", label: "Objection Handling", type: "agent", description: "Drafts objection rebuttal addressing pricing/timing", modelOrTool: "Objection AI", position: { x: 2430, y: 80 }, targetIds: ["n18"] },
    { id: "n16", label: "Calendar Availability", type: "action", description: "Checks executive calendar for open demo slots", modelOrTool: "Cal.com API", position: { x: 2430, y: 220 }, targetIds: ["n17"] },
    { id: "n17", label: "Book Meeting", type: "action", description: "Sends calendar invitation & updates meeting schedule", modelOrTool: "Google Calendar", position: { x: 2600, y: 220 }, targetIds: ["n18"] },
    { id: "n18", label: "CRM Update", type: "action", description: "Pushes prospect record, deal stage, and activity log to CRM", modelOrTool: "HubSpot / Salesforce", position: { x: 2770, y: 150 }, targetIds: ["n19"] },
