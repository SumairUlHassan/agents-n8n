...
export const marketingAgentConfig: AgentConfig = {
  ...
  workflowNodes: [
    { id: "n1", label: "Campaign Brief", type: "trigger", description: "Receives marketing parameters, brand voice & target audience", modelOrTool: "Form Gateway", position: { x: 50, y: 150 }, targetIds: ["n2"] },
    { id: "n2", label: "Input Validation", type: "action", description: "Validates campaign objective, offer details & platform specs", modelOrTool: "Validation Engine", position: { x: 220, y: 150 }, targetIds: ["n3"] },
    { id: "n3", label: "Audience Research", type: "agent", description: "Analyzes ICP pain points, emotional triggers & online behavior", modelOrTool: "Audience AI", position: { x: 390, y: 150 }, targetIds: ["n4"] },
    { id: "n4", label: "Competitor Research", type: "action", description: "Scrapes competitor ads, messaging hooks & social engagement", modelOrTool: "Ad Scraper", position: { x: 560, y: 150 }, targetIds: ["n5"] },
    { id: "n5", label: "Trend Analysis", type: "action", description: "Fetches trending keywords & hashtag sentiment in industry", modelOrTool: "Trend Engine", position: { x: 730, y: 150 }, targetIds: ["n6"] },
    { id: "n6", label: "Positioning Agent", type: "agent", description: "Defines unique value proposition & core campaign angle", modelOrTool: "Positioning AI", position: { x: 900, y: 150 }, targetIds: ["n7"] },
    { id: "n7", label: "Campaign Strategy", type: "agent", description: "Maps multi-stage funnel strategy (Awareness -> Intent -> Demo)", modelOrTool: "Strategy AI", position: { x: 1070, y: 150 }, targetIds: ["n8"] },
    { id: "n8", label: "Content Pillar Generator", type: "action", description: "Creates 4 strategic content pillars (Educational, Case Study, ROI, Product)", modelOrTool: "Pillar Engine", position: { x: 1240, y: 150 }, targetIds: ["n9"] },
    { id: "n9", label: "Copy Generator", type: "agent", description: "Drafts 10 LinkedIn posts, ad variants & email copy", modelOrTool: "Gemini 1.5 Pro", position: { x: 1410, y: 150 }, targetIds: ["n10"] },
    { id: "n10", label: "Creative Brief Generator", type: "action", description: "Generates image & carousel prompts for design team", modelOrTool: "Creative AI", position: { x: 1580, y: 150 }, targetIds: ["n11"] },
    { id: "n11", label: "Brand Voice Check", type: "agent", description: "Evaluates ad copy alignment against target brand guidelines", modelOrTool: "Voice Guardrail", position: { x: 1750, y: 150 }, targetIds: ["n12"] },
    { id: "n12", label: "Compliance Review", type: "action", description: "Verifies advertising guidelines & claim substantiation", modelOrTool: "Compliance Gate", position: { x: 1920, y: 150 }, targetIds: ["n13"] },
    { id: "n13", label: "Platform Formatter", type: "action", description: "Formats character limits, line breaks & hashtags per platform", modelOrTool: "Formatter Engine", position: { x: 2090, y: 150 }, targetIds: ["n14"] },
    { id: "n14", label: "Publishing Scheduler", type: "action", description: "Schedules post delivery slots across 30-day calendar", modelOrTool: "Buffer / Hootsuite API", position: { x: 2260, y: 150 }, targetIds: ["n15"] },
    { id: "n15", label: "Performance Data", type: "action", description: "Connects ad analytics API for impression tracking", modelOrTool: "LinkedIn Ads API", position: { x: 2430, y: 150 }, targetIds: ["n16"] },
    { id: "n16", label: "KPI Analysis", type: "agent", description: "Predicts reach, clicks, CTR & cost-per-lead (CPL)", modelOrTool: "KPI Model", position: { x: 2600, y: 150 }, targetIds: ["n17"] },
    { id: "n17", label: "Optimization Recommendations", type: "agent", description: "Suggests A/B testing variations for low-performing hooks", modelOrTool: "Optimizer AI", position: { x: 2770, y: 150 }, targetIds: ["n18"] },
    { id: "n18", label: "Report Generator", type: "action", description: "Renders campaign master brief & downloadable content kit", modelOrTool: "Report Engine", position: { x: 2940, y: 150 }, targetIds: ["n19"] },
    { id: "n19", label: "Audit Log", type: "output", description: "Stores campaign configuration & content assets in record store", modelOrTool: "Audit Engine", position: { x: 3110, y: 150 }, targetIds: [] },
  ],
  demoScenarios: [
    {
