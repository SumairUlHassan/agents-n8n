...
export const recruitingAgentConfig: AgentConfig = {
  ...
  workflowNodes: [
    { id: "n1", label: "Candidate Submission", type: "trigger", description: "Receives resume upload and job requisition details", modelOrTool: "Form / File Upload", position: { x: 50, y: 150 }, targetIds: ["n2"] },
    { id: "n2", label: "Resume Parser", type: "action", description: "Parses PDF/DOCX into clean structured JSON", modelOrTool: "pdf-parse / Unstructured", position: { x: 220, y: 150 }, targetIds: ["n3"] },
    { id: "n3", label: "Contact Extraction", type: "action", description: "Extracts contact information, email, phone, and LinkedIn", modelOrTool: "Regex / NLP Parser", position: { x: 390, y: 150 }, targetIds: ["n4"] },
    { id: "n4", label: "Skills Extraction", type: "agent", description: "Extracts technical & soft skills from work history", modelOrTool: "LLM Extractor", position: { x: 560, y: 150 }, targetIds: ["n5"] },
    { id: "n5", label: "Experience Analysis", type: "agent", description: "Quantifies relevant years of experience and domain tenure", modelOrTool: "Tenure Analyzer AI", position: { x: 730, y: 150 }, targetIds: ["n6"] },
    { id: "n6", label: "Job Requirement Parser", type: "action", description: "Breaks job description into mandatory vs nice-to-have criteria", modelOrTool: "Req Parser", position: { x: 900, y: 150 }, targetIds: ["n7"] },
    { id: "n7", label: "Candidate Matching", type: "agent", description: "Compares parsed candidate profile against job requirements", modelOrTool: "Embedding Matcher", position: { x: 1070, y: 150 }, targetIds: ["n8"] },
    { id: "n8", label: "Bias-Safety Check", type: "action", description: "Removes PII & ensures compliance with EEO guidelines", modelOrTool: "De-bias Guardrail", position: { x: 1240, y: 150 }, targetIds: ["n9"] },
    { id: "n9", label: "Candidate Score", type: "agent", description: "Calculates overall candidate suitability score (0-100)", modelOrTool: "Scoring Matrix", position: { x: 1410, y: 150 }, targetIds: ["n10"] },
    { id: "n10", label: "Score Branch", type: "decision", description: "Branches: Strong Match (>= 80) vs Manual Review vs Reject", modelOrTool: "Score Router", position: { x: 1580, y: 150 }, targetIds: ["n11"] },
    { id: "n11", label: "Generate Screening Questions", type: "agent", description: "Generates 5 role-specific interview questions based on resume gaps", modelOrTool: "Gemini 1.5 Pro", position: { x: 1750, y: 150 }, targetIds: ["n12"] },
    { id: "n12", label: "Candidate Outreach", type: "action", description: "Sends candidate email invite for preliminary screening", modelOrTool: "Email Sender", position: { x: 1920, y: 150 }, targetIds: ["n13"] },
    { id: "n13", label: "Screening Response Analysis", type: "agent", description: "Analyzes screening response for availability & expectations", modelOrTool: "Response Analyzer", position: { x: 2090, y: 150 }, targetIds: ["n14"] },
    { id: "n14", label: "Interview Eligibility", type: "decision", description: "Verifies scheduling criteria before calendar lookup", modelOrTool: "Eligibility Gate", position: { x: 2260, y: 150 }, targetIds: ["n15"] },
    { id: "n15", label: "Calendar Check", type: "action", description: "Finds engineering hiring manager calendar openings", modelOrTool: "Google Calendar API", position: { x: 2430, y: 150 }, targetIds: ["n16"] },
    { id: "n16", label: "Schedule Interview", type: "action", description: "Books 45-min technical interview & sends calendar invite", modelOrTool: "Cal.com API", position: { x: 2600, y: 150 }, targetIds: ["n17"] },
    { id: "n17", label: "ATS Update", type: "action", description: "Updates candidate stage to 'Technical Screen Scheduled'", modelOrTool: "Greenhouse / Lever ATS", position: { x: 2770, y: 150 }, targetIds: ["n18"] },
    { id: "n18", label: "Recruiter Notification", type: "action", description: "Sends evaluation summary to recruiter via Slack", modelOrTool: "Slack Webhook", position: { x: 2940, y: 150 }, targetIds: ["n19"] },
    { id: "n19", label: "Audit Log", type: "output", description: "Logs full evaluation scorecard to immutable record", modelOrTool: "Audit Store", position: { x: 3110, y: 150 }, targetIds: [] },
  ],
  demoScenarios: [
    {
