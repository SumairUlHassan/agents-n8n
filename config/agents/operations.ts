...
export const operationsAgentConfig: AgentConfig = {
  ...
  workflowNodes: [
    ...
    { id: "n5", label: "Loop Over SKUs", type: "action", kind: "code", description: "Queries live warehouse inventory levels for requested SKUs", modelOrTool: "WMS DB API", position: { x: 730, y: 150 }, targetIds: ["n6"], targetLabels: { "n6": "sku batch" } },
    { id: "n6", label: "IF: Stock Available?", type: "decision", kind: "if", description: "Branches: In Stock vs Low Stock / Out of Stock", modelOrTool: "Stock Router", position: { x: 900, y: 150 }, targetIds: ["n7", "n9"], targetLabels: { "n7": "low stock / reorder", "n9": "in stock" } },
    { id: "n7", label: "Supplier Lookup", type: "action", kind: "http", description: "Identifies backup supplier with fastest lead time", modelOrTool: "Supplier DB", position: { x: 1070, y: 70 }, targetIds: ["n8"] },
    { id: "n8", label: "Purchase Request", type: "action", kind: "http", description: "Issues auto PO to primary supplier for restock", modelOrTool: "NetSuite PO API", position: { x: 1240, y: 70 }, targetIds: ["n9"] },
    { id: "n9", label: "Payment Verification", type: "action", kind: "http", description: "Verifies credit card payment capture or Net-30 invoice term", modelOrTool: "Stripe API", position: { x: 1410, y: 150 }, targetIds: ["n10"] },
    { id: "n10", label: "Fulfillment Assignment", type: "action", kind: "http", description: "Assigns order picking task to nearest regional fulfillment hub", modelOrTool: "ShipStation API", position: { x: 1580, y: 150 }, targetIds: ["n11"] },
    { id: "n11", label: "Shipment Creation", type: "action", kind: "http", description: "Generates shipping label & tracking number (UPS / DHL)", modelOrTool: "EasyPost API", position: { x: 1750, y: 150 }, targetIds: ["n12"] },
    { id: "n12", label: "Delivery Tracking", type: "action", kind: "schedule", description: "Subscribes tracking webhook for real-time transit updates", modelOrTool: "Tracking Stream", position: { x: 1920, y: 150 }, targetIds: ["n13"] },
    { id: "n13", label: "Switch: Delay Detector", type: "agent", kind: "if", description: "Monitors carrier delays & predicts delivery SLA exceptions", modelOrTool: "Delay Predictor AI", position: { x: 2090, y: 150 }, targetIds: ["n14"], targetLabels: { "n14": "delay flagged" } },
    { id: "n14", label: "Exception Resolution", type: "agent", kind: "code", description: "Executes auto-reroute or upgrades shipping tier for delayed items", modelOrTool: "Resolution AI", position: { x: 2260, y: 150 }, targetIds: ["n15"] },
    { id: "n15", label: "Customer Notification", type: "action", kind: "http", description: "Sends proactive SMS / Email tracking update to buyer", modelOrTool: "Twilio / SendGrid", position: { x: 2430, y: 150 }, targetIds: ["n16"] },
    { id: "n16", label: "CRM Update", type: "action", kind: "http", description: "Updates Salesforce customer account timeline", modelOrTool: "Salesforce API", position: { x: 2600, y: 150 }, targetIds: ["n17"] },
    { id: "n17", label: "ERP Update", type: "action", kind: "http", description: "Synchronizes inventory balance & revenue ledger in ERP", modelOrTool: "SAP / NetSuite", position: { x: 2770, y: 150 }, targetIds: ["n18"] },
    { id: "n18", label: "Slack Alert", type: "action", kind: "http", description: "Posts status update to #ops-fulfillment channel", modelOrTool: "Slack Webhook", position: { x: 2940, y: 150 }, targetIds: ["n19"] },
    { id: "n19", label: "Daily Report", type: "agent", kind: "code", description: "Aggregates daily fulfillment KPIs into operational digest", modelOrTool: "Report Generator", position: { x: 3110, y: 150 }, targetIds: ["n20"] },
    { id: "n20", label: "Audit Log", type: "output", kind: "trigger", description: "Writes immutable execution log to operations database", modelOrTool: "Audit Engine", position: { x: 3280, y: 150 }, targetIds: [] },
  ],
  demoScenarios: [
    {
