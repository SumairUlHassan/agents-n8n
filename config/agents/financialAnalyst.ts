...
export const financialAnalystAgentConfig: AgentConfig = {
  ...
  workflowNodes: [
    { id: "n1", label: "File Upload Trigger", type: "trigger", kind: "trigger", description: "Receives CSV, XLSX or PDF financial statement upload", modelOrTool: "File Upload Stream", position: { x: 50, y: 150 }, targetIds: ["n2"] },
    { id: "n2", label: "File Validation", type: "action", kind: "code", description: "Validates file encoding, MIME type, and size limits", modelOrTool: "Validator Engine", position: { x: 220, y: 150 }, targetIds: ["n3"] },
    { id: "n3", label: "Data Extraction", type: "action", kind: "code", description: "Extracts tabular rows, sheet structures, and invoice metadata", modelOrTool: "SheetJS / PDF-Table", position: { x: 390, y: 150 }, targetIds: ["n4"] },
    { id: "n4", label: "Schema Detection", type: "action", kind: "code", description: "Maps columns to date, transaction_id, category, amount, vendor", modelOrTool: "Schema AI", position: { x: 560, y: 150 }, targetIds: ["n5"] },
    { id: "n5", label: "Loop Over Ledger Rows", type: "action", kind: "code", description: "Handles missing values, currency conversions & date normalization", modelOrTool: "Data Cleaner", position: { x: 730, y: 150 }, targetIds: ["n6"], targetLabels: { "n6": "row batch" } },
    { id: "n6", label: "Transaction Classification", type: "agent", kind: "code", description: "Classifies expense items into GAAP/IFRS categories", modelOrTool: "Classifier AI", position: { x: 900, y: 150 }, targetIds: ["n7"] },
    { id: "n7", label: "Metric Calculation", type: "action", kind: "code", description: "Computes total revenue, COGS, OPEX, EBITDA, and gross margin", modelOrTool: "Math Engine", position: { x: 1070, y: 150 }, targetIds: ["n8"] },
    { id: "n8", label: "Variance Analysis", type: "action", kind: "code", description: "Compares current period figures against prior quarter benchmarks", modelOrTool: "Variance Engine", position: { x: 1240, y: 150 }, targetIds: ["n9"] },
    { id: "n9", label: "IF: Anomaly Detected?", type: "agent", kind: "if", description: "Flags outlier transactions (> 3 std dev) & duplicate invoices", modelOrTool: "Isolation Forest AI", position: { x: 1410, y: 150 }, targetIds: ["n10", "n12"], targetLabels: { "n10": "normal spend", "n12": "anomaly (>3 std dev)" } },
    { id: "n10", label: "Forecast Model", type: "agent", kind: "code", description: "Generates time-series trend model for next quarter revenue", modelOrTool: "Forecast Engine", position: { x: 1580, y: 230 }, targetIds: ["n11"] },
    { id: "n11", label: "Risk Scoring", type: "agent", kind: "code", description: "Assesses audit risk, cash burn rate & liquidity runway", modelOrTool: "Risk AI", position: { x: 1750, y: 230 }, targetIds: ["n13"] },
    { id: "n12", label: "Insight Generator", type: "agent", kind: "code", description: "Synthesizes executive commentary on cost-reduction opportunities", modelOrTool: "Gemini 1.5 Pro", position: { x: 1920, y: 70 }, targetIds: ["n13"] },
    { id: "n13", label: "Executive Summary", type: "agent", kind: "code", description: "Drafts 1-page CFO memorandum with chart recommendations", modelOrTool: "Memo Generator", position: { x: 2090, y: 150 }, targetIds: ["n14"] },
    { id: "n14", label: "Report Builder", type: "action", kind: "http", description: "Renders printable financial report preview and summary cards", modelOrTool: "PDF / HTML Generator", position: { x: 2260, y: 150 }, targetIds: ["n15"] },
    { id: "n15", label: "Switch: Audit Gate", type: "decision", kind: "if", description: "Branches: High Anomaly Risk vs Approved Auto-Export", modelOrTool: "Audit Gate", position: { x: 2430, y: 150 }, targetIds: ["n16"], targetLabels: { "n16": "auto-approved" } },
    { id: "n16", label: "Export Results", type: "action", kind: "http", description: "Generates downloadable PDF audit report & clean CSV dataset", modelOrTool: "Exporter API", position: { x: 2600, y: 150 }, targetIds: ["n17"] },
    { id: "n17", label: "Audit Log", type: "output", kind: "trigger", description: "Records mathematical audit trail & compliance metadata", modelOrTool: "Audit Store", position: { x: 2770, y: 150 }, targetIds: [] },
  ],
  demoScenarios: [
    {
