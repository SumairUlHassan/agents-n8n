...
export const researchAgentConfig: AgentConfig = {
  ...
  workflowNodes: [
    { id: "n1", label: "Research Request", type: "trigger", description: "Receives research question & depth parameters", modelOrTool: "Query Gateway", position: { x: 50, y: 150 }, targetIds: ["n2"] },
    { id: "n2", label: "Query Decomposition", type: "agent", description: "Breaks broad topic into 5 targeted search sub-queries", modelOrTool: "Decomposer AI", position: { x: 220, y: 150 }, targetIds: ["n3"] },
    { id: "n3", label: "Research Plan", type: "action", description: "Drafts research plan specifying target domains & inclusion criteria", modelOrTool: "Planner Engine", position: { x: 390, y: 150 }, targetIds: ["n4"] },
    { id: "n4", label: "Source Search", type: "action", description: "Queries Google Scholar, PubMed, ArXiv, and web search APIs", modelOrTool: "Serper / Tavily API", position: { x: 560, y: 150 }, targetIds: ["n5"] },
    { id: "n5", label: "Web Retrieval", type: "action", description: "Scrapes full text content from top search result URLs", modelOrTool: "Firecrawl Scraper", position: { x: 730, y: 150 }, targetIds: ["n6"] },
    { id: "n6", label: "Document Retrieval", type: "action", description: "Parses PDF whitepapers, Gartner reports & clinical studies", modelOrTool: "PDF Reader AI", position: { x: 900, y: 150 }, targetIds: ["n7"] },
    { id: "n7", label: "Source Quality Scoring", type: "agent", description: "Scores domain authority, publish date, author & peer review status", modelOrTool: "Authority Evaluator", position: { x: 1070, y: 150 }, targetIds: ["n8"] },
    { id: "n8", label: "Duplicate Removal", type: "action", description: "Deduplicates redundant articles & syndicated press releases", modelOrTool: "Deduper Engine", position: { x: 1240, y: 150 }, targetIds: ["n9"] },
    { id: "n9", label: "Fact Extraction", type: "agent", description: "Extracts key statistical metrics, quotes & findings from text", modelOrTool: "Fact Extractor AI", position: { x: 1410, y: 150 }, targetIds: ["n10"] },
    { id: "n10", label: "Claim Verification", type: "agent", description: "Cross-checks facts against secondary independent sources", modelOrTool: "Verifier AI", position: { x: 1580, y: 150 }, targetIds: ["n11"] },
    { id: "n11", label: "Contradiction Detection", type: "agent", description: "Identifies conflicting data points (e.g. 40% vs 25% adoption)", modelOrTool: "Contradiction AI", position: { x: 1750, y: 150 }, targetIds: ["n12"] },
    { id: "n12", label: "Evidence Grouping", type: "action", description: "Clusters findings into logical thematic chapters", modelOrTool: "Clustering AI", position: { x: 1920, y: 150 }, targetIds: ["n13"] },
    { id: "n13", label: "Synthesis Agent", type: "agent", description: "Synthesizes multi-source evidence into coherent narrative", modelOrTool: "Gemini 1.5 Pro", position: { x: 2090, y: 150 }, targetIds: ["n14"] },
    { id: "n14", label: "Citation Builder", type: "action", description: "Appends APA formatted reference list & inline index numbers", modelOrTool: "Citation Engine", position: { x: 2260, y: 150 }, targetIds: ["n15"] },
    { id: "n15", label: "Report Writer", type: "agent", description: "Compiles executive research document with tables & charts", modelOrTool: "Writer AI", position: { x: 2430, y: 150 }, targetIds: ["n16"] },
    { id: "n16", label: "Quality Review", type: "action", description: "Checks hallucination rate & citation mapping accuracy", modelOrTool: "Quality Guardrail", position: { x: 2600, y: 150 }, targetIds: ["n17"] },
    { id: "n17", label: "Export Report", type: "action", description: "Generates downloadable PDF research report", modelOrTool: "Exporter API", position: { x: 2770, y: 150 }, targetIds: ["n18"] },
    { id: "n18", label: "Audit Log", type: "output", description: "Logs full source list & verification trail to database", modelOrTool: "Audit Engine", position: { x: 2940, y: 150 }, targetIds: [] },
  ],
  demoScenarios: [
    {
