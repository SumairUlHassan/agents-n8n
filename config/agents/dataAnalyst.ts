...
export const dataAnalystAgentConfig: AgentConfig = {
  ...
  workflowNodes: [
    { id: "n1", label: "Dataset Upload", type: "trigger", kind: "trigger", description: "Receives raw dataset file (CSV/XLSX/JSON) and user query", modelOrTool: "Data Ingest Gate", position: { x: 50, y: 150 }, targetIds: ["n2"] },
    { id: "n2", label: "File Validation", type: "action", kind: "code", description: "Validates column headers, delimiters, and file integrity", modelOrTool: "Validator Engine", position: { x: 220, y: 150 }, targetIds: ["n3"] },
    { id: "n3", label: "Schema Detection", type: "action", kind: "code", description: "Infers data types (numeric, categorical, datetime, boolean)", modelOrTool: "Schema Detector", position: { x: 390, y: 150 }, targetIds: ["n4"] },
    { id: "n4", label: "Data Profiling", type: "action", kind: "code", description: "Calculates null percentages, distributions, mean, std dev", modelOrTool: "Pandas Profiler AI", position: { x: 560, y: 150 }, targetIds: ["n5"] },
    { id: "n5", label: "Loop Over Row Chunks", type: "action", kind: "code", description: "Identifies null values and blank records in chunks", modelOrTool: "Null Inspector", position: { x: 730, y: 150 }, targetIds: ["n6"], targetLabels: { "n6": "row chunk" } },
    { id: "n6", label: "Data Cleaning", type: "action", kind: "code", description: "Imputes missing values with median/mode & drops corrupted rows", modelOrTool: "Auto-Cleaner", position: { x: 900, y: 150 }, targetIds: ["n7"] },
    { id: "n7", label: "Type Conversion", type: "action", kind: "code", description: "Casts strings to timestamps & float amounts", modelOrTool: "Type Converter", position: { x: 1070, y: 150 }, targetIds: ["n8"] },
    { id: "n8", label: "Statistical Analysis", type: "action", kind: "code", description: "Computes Pearson correlation matrix & ANOVA variance", modelOrTool: "Stats Engine", position: { x: 1240, y: 150 }, targetIds: ["n9"] },
    { id: "n9", label: "Switch: NL2SQL Router", type: "agent", kind: "if", description: "Translates natural language question into SQL / Pandas filter", modelOrTool: "NL2SQL AI", position: { x: 1410, y: 150 }, targetIds: ["n10"], targetLabels: { "n10": "SQL generated" } },
    { id: "n10", label: "Metric Selection", type: "action", kind: "code", description: "Extracts key performance indicators relevant to inquiry", modelOrTool: "KPI Selector", position: { x: 1580, y: 150 }, targetIds: ["n11"] },
    { id: "n11", label: "Pattern Detection", type: "agent", kind: "code", description: "Discovers significant trends, growth spikes & correlations", modelOrTool: "Pattern AI", position: { x: 1750, y: 150 }, targetIds: ["n12"] },
    { id: "n12", label: "IF: Outlier Flagged?", type: "agent", kind: "if", description: "Detects statistical outliers (> 3 std dev above mean)", modelOrTool: "Anomaly AI", position: { x: 1920, y: 150 }, targetIds: ["n13"], targetLabels: { "n13": "outlier flagged" } },
    { id: "n13", label: "Segmentation", type: "action", kind: "code", description: "Clusters rows into High Value, At-Risk & Core segments", modelOrTool: "K-Means Engine", position: { x: 2090, y: 150 }, targetIds: ["n14"] },
    { id: "n14", label: "Insight Generation", type: "agent", kind: "code", description: "Synthesizes executive business takeaways & action items", modelOrTool: "Gemini 1.5 Pro", position: { x: 2260, y: 150 }, targetIds: ["n15"] },
    { id: "n15", label: "Visualization Selection", type: "agent", kind: "code", description: "Selects optimal bar, line & pie charts to display insights", modelOrTool: "Chart Config AI", position: { x: 2430, y: 150 }, targetIds: ["n16"] },
    { id: "n16", label: "Executive Summary", type: "agent", kind: "code", description: "Renders interactive analytics dashboard layout", modelOrTool: "Dashboard Generator", position: { x: 2600, y: 150 }, targetIds: ["n17"] },
    { id: "n17", label: "Export Analysis", type: "action", kind: "http", description: "Generates downloadable summary PDF & CSV clean dataset", modelOrTool: "Exporter API", position: { x: 2770, y: 150 }, targetIds: ["n18"] },
    { id: "n18", label: "Audit Log", type: "output", kind: "trigger", description: "Logs data transformation steps & code execution for auditability", modelOrTool: "Audit Engine", position: { x: 2940, y: 150 }, targetIds: [] },
  ],
  demoScenarios: [
    {
      id: "da-sales-drivers",
