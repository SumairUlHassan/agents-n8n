...
export const executiveAssistantAgentConfig: AgentConfig = {
  ...
  workflowNodes: [
    { id: "n1", label: "Request Trigger", type: "trigger", description: "Receives executive command or automated scheduled trigger", modelOrTool: "Voice / Text Input", position: { x: 50, y: 150 }, targetIds: ["n2"] },
    { id: "n2", label: "Intent Classification", type: "agent", description: "Classifies task intent: Inbox, Schedule, Draft, Briefing", modelOrTool: "Intent Classifier", position: { x: 220, y: 150 }, targetIds: ["n3"] },
    { id: "n3", label: "Permission Check", type: "action", description: "Verifies authorization & policy scope for requested action", modelOrTool: "Policy Engine", position: { x: 390, y: 150 }, targetIds: ["n4"] },
    { id: "n4", label: "Email Search", type: "action", description: "Fetches recent inbox threads, unread emails, and high-priority flags", modelOrTool: "Gmail / Outlook API", position: { x: 560, y: 150 }, targetIds: ["n5"] },
    { id: "n5", label: "Priority Scoring", type: "agent", description: "Scores email urgency based on sender, keywords, and SLA", modelOrTool: "Priority AI", position: { x: 730, y: 150 }, targetIds: ["n6"] },
    { id: "n6", label: "Thread Summarization", type: "agent", description: "Compresses multi-turn email chains into key action items", modelOrTool: "Gemini Flash AI", position: { x: 900, y: 150 }, targetIds: ["n7"] },
    { id: "n7", label: "Calendar Search", type: "action", description: "Queries calendar for conflict-free availability slots", modelOrTool: "Google Calendar API", position: { x: 1070, y: 150 }, targetIds: ["n8"] },
    { id: "n8", label: "Conflict Detection", type: "action", description: "Identifies double-bookings & timezone overlaps", modelOrTool: "Conflict Analyzer", position: { x: 1240, y: 150 }, targetIds: ["n9"] },
    { id: "n9", label: "Time-Slot Selection", type: "agent", description: "Selects optimal 30-min window respecting executive focus time", modelOrTool: "Scheduler AI", position: { x: 1410, y: 150 }, targetIds: ["n10"] },
    { id: "n10", label: "Draft Generator", type: "agent", description: "Drafts executive-toned response with proposed agenda", modelOrTool: "Gemini 1.5 Pro", position: { x: 1580, y: 150 }, targetIds: ["n11"] },
    { id: "n11", label: "Approval Required Branch", type: "decision", description: "Requires executive sign-off before sending email or booking", modelOrTool: "Human-in-the-Loop", position: { x: 1750, y: 150 }, targetIds: ["n12", "n13"] },
    { id: "n12", label: "Create Calendar Event", type: "action", description: "Creates calendar invite & attaches briefing notes", modelOrTool: "Calendar API", position: { x: 1920, y: 80 }, targetIds: ["n14"] },
    { id: "n13", label: "Create Tasks", type: "action", description: "Adds action items to Notion / Todoist workspace", modelOrTool: "Notion API", position: { x: 1920, y: 220 }, targetIds: ["n14"] },
    { id: "n14", label: "Update Notes", type: "action", description: "Appends thread summary to Executive Daily Log", modelOrTool: "Log Store", position: { x: 2090, y: 150 }, targetIds: ["n15"] },
    { id: "n15", label: "Send Notification", type: "action", description: "Delivers push notification summary to executive phone", modelOrTool: "Pushover / Slack", position: { x: 2260, y: 150 }, targetIds: ["n16"] },
    { id: "n16", label: "Audit Log", type: "output", description: "Records timestamped audit entry of executive actions", modelOrTool: "Audit Engine", position: { x: 2430, y: 150 }, targetIds: [] },
  ],
  demoScenarios: [
    {
